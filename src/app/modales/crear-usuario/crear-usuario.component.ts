...
export class CrearUsuarioComponent extends DialogComponent<PromptModel, any> implements PromptModel {
  scope: any;
  agregarlo: string = 'si';
  currentRequest: any;
  userSolicitud: User;
...
segundoStep(){
  this.userToEdit = this.scope.userLogged;
...
guardarCambios(){
    ...
    if (this.croppedImage) {
      ...
      var fecha =new Date(d.toISOString().replace("Z", "-02:00")).toISOString().replace(".000", "");
      this.secondFormGroup.get('fechaCreacion').setValue(fecha);
      this.userToEdit.fechaCreacion = this.secondFormGroup.get('fechaCreacion').value; 
